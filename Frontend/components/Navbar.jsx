...
const BookIcon = () => (
    <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 
        5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>
    </svg>
);
    ...
    return (
        <nav className={`fixed top-0 left-0 bg-indigo-500 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>            
            ...
            <div className="hidden md:flex items-center gap-4 lg:gap-8">
                ...
                {user && (
                    <button 
                        onClick={() => isOwner ? navigate('/owners') : setShowHotelReg?.(true)} 
                ...
                <div className="flex items-center gap-2 relative">
                    {showSearch && (
                        <input
                            ...
                            className={`px-3 py-1 text-sm rounded-full border outline-none transition-all duration-300 ${
                                isScrolled ? "border-gray-300 text-black bg-white" : "border-white/50 text-white bg-white/20 placeholder-white/80"
                            }`}
                            autoFocus
                        />
                    )}
                    ...
                    <svg 
                        ...
                        className={`h-6 w-6 cursor-pointer transition-all duration-500 ${isScrolled ? "invert text-black" : "text-white"}`} 
                        fill="none" 
                        stroke="currentColor" 
                        strokeWidth="2" 
                        viewBox="0 0 24 24"
            ...
            <div className={`fixed top-0 left-0 w-full h-screen bg-white text-base flex flex-col md:hidden items-center justify-center gap-6 font-medium text-gray-800 transition-all duration-500 z-50 ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
                ...
                {user && (
                    <button onClick={() => { navigate(isOwner ? '/owners' : '/'); setIsMenuOpen(false); }} className="border border-gray-400 px-4 py-1 text-sm font-light rounded-full cursor-pointer transition-all">
                        {isOwner ? 'Dashboard' : 'List your Hotel'}
                    </button>
                )}

                {!user ? (
                    <button onClick={() => { openSignIn(); setIsMenuOpen(false); }} className="bg-black text-white px-8 py-2.5 rounded-full transition-all duration-500">
                        Login
                    </button>
                ) : (
                    <UserButton afterSignOutUrl="/">
                        <UserButton.MenuItems>
                            <UserButton.Action label="My Bookings" labelIcon={<BookIcon />} onClick={() => { navigate('/my-bookings'); setIsMenuOpen(false); }} />
                        </UserButton.MenuItems>
                    </UserButton>
                )}
            </div>
