...
const Navbar = () => {
    ...
    React.useEffect(() => {
        ...
        // Initialize state right away based on path
        if (location.pathname !== '/') {
            setIsScrolled(true);
        }
        ...
        <div ref={ref} className="h-88 md:h-64 overflow-y-scroll">
            ...
            <nav className={`fixed top-0 left-0 bg-indigo-500 w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
                ...
                <div className="hidden md:flex items-center gap-4">
                    <svg className={`h-6 w-6 text-white transition-all duration-500 ${isScrolled ? "invert text-black" : ""}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                        <circle cx="11" cy="11" r="8" />
                        <line x1="21" y1="21" x2="16.65" y2="16.65" />
                    </svg>
                    
                    {/* Clerk UserButton / Login Toggle */}
                    {user ? (
                        <UserButton />
                    ) : (
                ...
                <div className={`fixed top-0 left-0 w-full h-screen bg-white text-base flex flex-col md:hidden items-center justify-center gap-6 font-medium text-gray-800 transition-all duration-500 z-50 ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
                    ...
                    <button onClick={() => { navigate('/dashboard'); setIsMenuOpen(false); }} className="border border-gray-400 px-4 py-1 text-sm font-light rounded-full cursor-pointer transition-all">
                        Dashboard
                    </button>
                    ...
                    {!user ? (
                        ...
                        <UserButton afterSignOutUrl="/">
                            <UserButton.MenuItems>
                                <UserButton.Action label="My Bookings" labelIcon={<BookIcon />} onClick={() => { navigate('/bookings'); setIsMenuOpen(false); }} />
                            </UserButton.MenuItems>
                        </UserButton>
                    )}
                </div>
